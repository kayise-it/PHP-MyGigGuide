...
<div class="p-6 max-w-2xl">
    ...
    @if($totalVotes > 0)
        <div class="mb-6 rounded-lg border border-white/10 bg-slate-900/70 p-5">
            <h3 class="text-sm font-semibold text-slate-300 mb-3 uppercase tracking-wide">Current Results ({{ $totalVotes }} votes)</h3>
            @foreach($poll->resultsArray() as $result)
                <div class="mb-3">
                    <div class="flex justify-between text-sm mb-1">
                        <span class="text-slate-200">{{ $result['label'] }}</span>
                        <span class="text-slate-400">{{ $result['votes'] }} ({{ $result['percent'] }}%)</span>
                    </div>
                    <div class="h-2 rounded-full bg-slate-700 overflow-hidden">
                        <div class="h-full rounded-full bg-indigo-500" style="width: {{ $result['percent'] }}%"></div>
                    </div>
                </div>
            @endforeach
        </div>
